...
export class BulkActionsRoutesV2 {
    ...
    private async loadRoutes() {
        this.router.post(`/ba/addGameVersion`, async (req, res) => {
            ...
            */
            ...
            if (await Validator.validateIDArray(modVersionIds.data, `versions`, false, false) == false) {
            ...
            for (let modVersion of modVersions) {
                let outObj = await modVersion.addGameVersionId(gameVersion.id, session.user).catch((err:any) => {
        ...
        this.router.post(`/ba/linkVersionsExclude`, async (req, res) => {
            ...
            */
            ...
            if (await Validator.validateIDArray(modVersionIds.data, `versions`, true, true) == false) {
                res.status(404).send({ message: `One or more mod versions not found`});
                return;
            }
            ...
            for (let modVersion of modVersions) {
                ...
                let existing = modVersionFiltered.find((mv) => mv.modId === modVersion.projectId && mv.modVersion.status === modVersion.status);
                if (existing) {
                    if (modVersion.modVersion.compare(existing.modVersion.modVersion) == 1) {
                        modVersionFiltered = modVersionFiltered.filter((mv) => mv.modId !== modVersion.projectId && mv.modVersion.status === modVersion.status);
