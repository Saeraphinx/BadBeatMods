...
import { DatabaseHelper, DatabaseManager, EditQueue, GameVersionInfer, GameWebhookConfig, Platform, Project, ProjectAPIPublicResponseV2, ProjectInfer, Status, SupportedGames, User, UserInfer, UserRoles, Version, VersionAPIPublicResponseV2, VersionInfer } from '../../src/shared/Database.ts';
// #region setup
const api = supertest(`http://localhost:8497/api/v2`);
...
import { ApprovalAction } from '../../src/api/routes/v2/approval.ts';
...
vi.mock(import(`../../src/shared/ModWebhooks.ts`), async (importOriginal) => {
    ...
    return {
        ...
        sendVersionLog: vi.fn(async (version: Version, userMakingChanges: User, logType: WebhookLogType, projectObj?: Project, reason?: string) => { }),
...
describe.sequential(`API v2`, async () => {
    ...
    describe.sequential(`Get Mods`, () => {
        ...
        test(`/mods - gv only`, async () => {
            ...
            for (let cmod of response.body.mods) {
                let currentMod = cmod as { mod: ProjectAPIPublicResponseV2, latest: VersionAPIPublicResponseV2 };
                expect(currentMod).toHaveProperty(`mod`);
                expect(currentMod).toHaveProperty(`latest`);
    ...
    describe.skip.sequential(`Edit Mods`, () => {
        beforeEach(() => {
            shouldAuthenticateWithRole = false;
        });
...
function testStatusChange(testMod: Project | Version, fromStatus: Status, action: ApprovalAction, toStatus: Status, logAction: Function) {
    return async () => {
        ...
        await testMod.save();
        let type = testMod instanceof Project ? `mod` : `modversion`;
